...
public class AnalyticsUtils {
    ...
    private static final String UACODE = "UA-27446305-1";
    ...
    private AnalyticsUtils(Context context) {
        ...
        this.mTracker.start(UACODE, 300, this.mApplicationContext);
        ...
        if (firstRun) {
            ...
            this.mTracker.setCustomVar(1, "apiLevel", apiLevel, VISITOR_SCOPE);
            this.mTracker.setCustomVar(2, "model", model, VISITOR_SCOPE);
            ...
            protected Void doInBackground(Void... voids) {
                try {
                	if (CurrentEnvironment.environment == AppEnvironment.Production){
	                    AnalyticsUtils.this.mTracker.trackEvent(category, action, label, value);
	                    Log.d(TAG, "isitbusythere?  Analytics trackEvent: "
	                            + category + " / " + action + " / " + label + " / " + value);
                	}
                } catch (Exception e) {
                    // We don't want to crash if there's an Analytics library exception.
                    Log.w(TAG, "isitbusythere?  Analytics trackEvent error: "
    ...
    public void trackPageView(final String path) {
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... voids) {
                try {
                	if (CurrentEnvironment.environment == AppEnvironment.Production){
	                    AnalyticsUtils.this.mTracker.trackPageView(path);
	                    Log.d(TAG, "isitbusythere?  Analytics trackPageView: " + path);
                	}
                } catch (Exception e) {
                    // We don't want to crash if there's an Analytics library exception.
                    Log.w(TAG, "isitbusythere?  Analytics trackPageView error: " + path, e);
                }
                return null;
            }
        }.execute();
